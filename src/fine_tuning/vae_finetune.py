...
def acc_unwrap_model(model):
    ...
    if hasattr(model, "module"):
        return acc_unwrap_model(model.module)
    # If the model is wrapped by torch's FSDP (FullyShardedDataParallel), return the original model.
        ...
        return acc_unwrap_model(model._fsdp_wrapped_module)
...
@torch.no_grad()
def log_validation(args, test_dataloader, vae, accelerator, weight_dtype, epoch=0, curr_step = 0, max_validation_sample=20):
    ...
    with accelerator.autocast():
        for i, sample in enumerate(test_dataloader):
            if i < max_validation_sample:
                ...
                x = x.unsqueeze(2)  # add the extra dim
                
                # reconstructions = vae_model(x).sample
                posterior = vae_model.encode(x).latent_dist
                z = posterior.sample()
                reconstructions = vae_model.decode(z).sample
                
                reconstructions = reconstructions.squeeze(2)  # remove the extra dim
                x = x.squeeze(2)
                
                imgs = torch.cat([x.cpu(), reconstructions.cpu()], dim=3)
                imgs = (imgs / 2 + 0.5).clamp(0, 1)
                
                images.append(imgs)

    # Create a grid from the collected images
    if len(images) > 0:
        # images list contains tensors of shape (1, 3, H, 2W)
        # Concatenate them to (N, 3, H, 2W)
        all_images = torch.cat(images, dim=0)
        
        # Create grid
        grid = torchvision.utils.make_grid(all_images, nrow=4)
        
        for tracker in accelerator.trackers:
            if tracker.name == "tensorboard":
                tracker.writer.add_image(
                    "Validation/Samples", grid, epoch
                )
            elif tracker.name == "wandb":
                tracker.log(
                    {"Validation/Samples": wandb.Image(grid)},
                    step=curr_step
                )
            else:
...
def parse_args():
    ...
    args.pretrained_model_name_or_path = r"/home/ckenny/models/FIBO/vae_bak/"
    args.revision = None
    args.dataset_name = None
    
    args.image_column = None
    args.output_dir = r"outputs/models_v4_bf16"
    args.seed = 420
    args.resolution = 256
    args.train_batch_size = 4 # batch 2 was the best for a single rtx3090
    args.num_train_epochs = 1
    args.gradient_accumulation_steps = 3
    args.gradient_checkpointing = False # not supported by KLWan
    ...
    args.use_ema = False # this will drastically slow your training, check speed vs performance
    ...
    args.train_data_dir = r"/home/ckenny/work/DV/datasets/cats/aligned/"
    args.test_data_dir = None
    args.checkpointing_steps = 400# return to 5000
...
def main():
    ...
    if args.use_ema:
        try:
            ema_vae = AutoencoderKLWan.from_pretrained(
                args.pretrained_model_name_or_path, subfolder="vae", revision=args.revision, torch_dtype=torch.float32)
        except:
            ema_vae = AutoencoderKLWan.from_pretrained(
                args.pretrained_model_name_or_path, revision=args.revision, torch_dtype=torch.float32)
        ema_vae = EMAModel(ema_vae.parameters(), model_cls=AutoencoderKLWan, model_config=ema_vae.config)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def save_model_hook(vae, weights, output_dir):
            ...
            vae = accelerator.unwrap_model(vae[0])
        ...
        def load_model_hook(models, input_dir):
            if args.use_ema:
                load_model = EMAModel.from_pretrained(os.path.join(input_dir, "vae_ema"), AutoencoderKLWan)
                ema_vae.load_state_dict(load_model.state_dict())
                ema_vae.to(accelerator.device, dtype=weight_dtype)
                del load_model

            # load diffusers style into model
            load_model = AutoencoderKLWan.from_pretrained(input_dir, subfolder="vae", weight_dtype=weight_dtype)
    ...
    if args.train_only_decoder:
        ...
        # Also freeze quant_conv as it is part of the encoding process
        if hasattr(vae, "quant_conv"):
            for param in vae.quant_conv.parameters():
                param.requires_grad = False
        # Also freeze post_quant_conv
    ...
    if accelerator.is_main_process:
        with torch.no_grad():
            log_validation(args, test_dataloader, vae, accelerator, weight_dtype, first_epoch, global_step)
        ...
        with accelerator.autocast():
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(vae):
                    ...
                    # reshape target to add another dim so it's batch x channels x 1 x height x width
                    ...
                    if accelerator.num_processes > 1:
                        ...
                        if debug or step < 5:
                            print(f"\n--- Latent Stats Step {step} ---")
                            print(f"z mean: {z.mean().item():.4f}, z std: {z.std().item():.4f}")
                            if hasattr(vae.config, "latents_mean"):
                                l_mean = torch.tensor(vae.config.latents_mean).to(z.device)
                                l_std = torch.tensor(vae.config.latents_std).to(z.device)
                                print(f"Config mean: {l_mean.mean().item():.4f}, Config std: {l_std.mean().item():.4f}")
                ...
                if accelerator.sync_gradients:
                    ...
                    if global_step % 10 == 0:
                        # Create grid: top row = target, bottom row = pred
                        # target and pred are (B, C, H, W)
                        # We want to show a few samples, say up to 4
                        n_samples = min(4, target.shape[0])
                        
                        # Concatenate target and pred for the selected samples
                        # shape: (2*n_samples, C, H, W)
                        imgs_to_log = torch.cat([target[:n_samples], pred[:n_samples]], dim=0)
                        
                        # Denormalize if needed (images are likely [-1, 1])
                        imgs_to_log = (imgs_to_log / 2 + 0.5).clamp(0, 1)
                        
                        grid = torchvision.utils.make_grid(imgs_to_log, nrow=n_samples)
                        
                        for tracker in accelerator.trackers:
                            if tracker.name == "tensorboard":
                                tracker.writer.add_image("Train/Samples", grid, global_step)

                    if global_step % args.checkpointing_steps == 0:
                        if accelerator.is_main_process:
                            with torch.no_grad():
                                log_validation(args, test_dataloader, vae, accelerator, weight_dtype, epoch, global_step)
                ...
                logs = {
                    "loss/step_loss": loss.detach().item(),
                    "lr": lr_scheduler.get_last_lr()[0],
                    "loss/mse": mse_loss.detach().item(),
